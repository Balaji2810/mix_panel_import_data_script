...
def import_data(user_id, event_name, time_stamp, props):
    mp.import_data(api_key, user_id, event_name, time_stamp, props, api_secret=api_key)
...
def fetch_and_upload_payments(skip=0):
    for count, payment in enumerate(
        ...
        import_data(payment.user, "Payment", payment.timestamp, props)
...
def fetch_and_upload_transactions(skip=0):
    for count, transaction in enumerate(
        ...
        import_data(transaction.user, "Transaction", transaction.timestamp, props)
